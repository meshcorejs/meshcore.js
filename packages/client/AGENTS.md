...
src/
  ...
  structures/                  Contact, Channel, Message (author: Contact | UnverifiedAuthor, backlog, snr…)
  messages/
  ...
  commands/
    ...
    command.ts                 Command (built definition + precomputed usages)
...
- `new Client({ transport, radio?: RadioConfig, replies?: Partial<Replies>, logger?, appName? = 'meshcore.js', load? })`
  ...
  `ClientStateError` before login), `client.radioConfig`.
...
- Errors: `MeshcoreError { code }` → `LoadError`, `ConnectionError`, `ClientStateError`,
  ...
  `RadioConfigError`, `LimitReachedError`, `MessageTooLongError`, `DeliveryFailedError`.
...
- Pipeline order: trigger → scope → maxAge (backlog) → permissions → cooldown → argument parsing → handler.
  Every refusal emits `commandDenied` with a typed `DenyReason`; `backlog` and `scope` are silent, the others
  answer with `client.replies` — **English by default**, `frenchReplies` preset, partial override. Only texts are
  configurable: command names and actions (`help`, `plugins`, `jobs`, `load`, `unload`, `reload`, `run`, `pause`,
  `resume`) stay English.
- Built-in helper (`/`, `/help`, `@Bot`, `@Bot help`): one syntax line per command, filtered by scope and
  (in DM) by the author's permissions; channel lists only permission-free commands.
...
- `pnpm vitest run packages/client`. Use `setupClient()` from `test/helpers.ts` (`MockTransport` +
  ...
  waits: send pacing, ACK timeouts, backoff, cooldowns, jobs.
...
- Assert what went on the air through `radio.sent` (`wireText` for channels), and refusals through
  `commandDenied`.
